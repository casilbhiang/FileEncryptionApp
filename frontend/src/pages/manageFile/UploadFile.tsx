...
const UploadFilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (storedUserId && storedUserUuid) {
      ...
      const handleKeyUpdate = (e: Event) => {
        ...
        if (customEvent.detail?.userId === storedUserId) {
          console.log('Key update event received, reloading key...');
          loadEncryptionKey(storedUserId);
        }
      };
      ...
      return () => {
        window.removeEventListener('encryption-key-updated', handleKeyUpdate);
      };
    } else {
  ...
  // Helper to validate file signature (magic numbers)
  const validateFileSignature = async (file: File): Promise<boolean> => {
    return new Promise((resolve) => {
      ...
      reader.onloadend = (e) => {
        ...
        console.log(`File: ${file.name}, Header: ${header}`);
  ...
  const handleFiles = async (files: File[]) => {
    ...
    // Filter files based on signature validation
    const validFiles: File[] = [];
    for (const file of files) {
      // Basic extension check first (for UX, though signature is authoritative)
      const ext = file.name.split('.').pop()?.toLowerCase();
      if (!['pdf', 'png', 'jpg', 'jpeg'].includes(ext || '')) {
        showErrorToast('Invalid File Type', `File ${file.name} has an invalid extension.`);
        continue;
      }

      const isValid = await validateFileSignature(file);
      if (!isValid) {
        showErrorToast('Security Alert', `File ${file.name} content does not match its extension. Upload rejected.`);
        console.error(`Signature validation failed for ${file.name}`);
        continue;
      }
