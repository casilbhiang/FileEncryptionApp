...
const BiometricModal: React.FC<BiometricModalProps> = ({ isOpen, onClose, onSuccess, userId, userName, mode }) => {
        ...
        try {
            if (modalMode === 'authenticate') {
                ...
                try {
                    ...
                    // If authentication fails because credential not in THIS browser
                    if (authError.message?.includes('not allowed') || 
                        authError.message?.includes('No biometric credentials')) {
                        // Switch to registration mode
                        setModalMode('register');
                        setError('No biometric registered on this browser. Please register first.');
                        setIsLoading(false);
                        return;
                    }
                    throw authError;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
      <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl relative animate-fadeIn">
        {/* Close button (only for registration mode) */}
        {mode === 'register' && (
          <button
            onClick={onClose}
        ...
        <h2 className="text-2xl font-bold text-gray-900 text-center mb-4">
          {mode === 'register' ? 'Register Biometric' : 'Biometric Authentication Required'}
        </h2>
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  {mode === 'register' ? 'Register Biometric' : 'Authenticate Now'}
                </>
              )}
            </button>
